...
const Home = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://localhost:1009/tasks');
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      if (editingTask) {
        ...
        const response = await axios.put(`http://localhost:1009/tasks/${editingTask._id}`, {
          title: newTask,
  ...
  const handleDeleteTask = async (id) => {
    try {
      await axios.delete(`http://localhost:1009/tasks/${id}`);
  ...
  const handleToggleComplete = async (task) => {
    try {
      const response = await axios.put(`http://localhost:1009/tasks/${task._id}`, {
        title: task.title,
